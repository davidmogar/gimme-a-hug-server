...
server.get('/hugs/:username', hugRoutes.countHugs);
...
server.del('/users/:username', userRoutes.deleteUser);
server.post('/users/:user1/follow/:user2', userRoutes.followUser);
server.get('/users', userRoutes.listUsers);
server.put('/users/:username', userRoutes.updateUser);
server.get('/users/:username', userRoutes.viewUser);
